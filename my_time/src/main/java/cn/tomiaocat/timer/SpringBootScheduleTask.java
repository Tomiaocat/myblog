...
public class SpringBootScheduleTask {
    ...
    Logger logger = LoggerFactory.getLogger(SpringBootScheduleTask.class);
    private static Integer ONE = 1;
    
    //    @Scheduled(cron = "0/1 * * * * ?")
    @Scheduled(cron = "32 17 9 * * ?")
    public void printSay() throws MessagingException {
        ...
        for (TimerUrl timerUrl : list) {
            ...
            String url = timerUrl.getUrl();
            SendEmail sendEmail = new SendEmail();
            try {
                //在这里写你要执行的内容
                int v = (int) (1 + Math.random() * (20 - 1 + 1)) * 50 * 1000;//先随机暂停1到 59分  任意一个时间段
                //随机睡眠一段时间
                Thread.sleep(v);
                ...
                SRepository.save(sendEmail);
                logger.info(timerUrl.getName()+"======>执行结束");
            } catch (Exception e) {
