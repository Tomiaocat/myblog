package cn.tomiaocat.util.date;

import com.eq.date.DateStyle;
import com.eq.date.Week;
...
public class DateUtil extends TagSupport {
    ...
    public static int getIntervalHours(Date date, Date otherDate) {
        date = com.eq.date.DateUtil.StringToDate(com.eq.date.DateUtil.getDate(date));
        long time = Math.abs(date.getTime() - otherDate.getTime());
    ...
    public static int getIntervalMinu(Date date, Date otherDate) {
    	date = com.eq.date.DateUtil.StringToDate(com.eq.date.DateUtil.DateToString(date, pattern_ymd_hms), pattern_ymd_hms);
    	long time = Math.abs(date.getTime() - otherDate.getTime());
	...
	public static String showText(Date date) {
		...
		Date lastSunday = com.eq.date.DateUtil.getSunday(Calendar.getInstance(), -1);
		
		
		// 上上周日.
		Calendar.getInstance().setTime(new Date());
		Date last2Sunday = com.eq.date.DateUtil.getSunday(Calendar.getInstance(), -2);
		...
		switch (com.eq.date.DateUtil.getIntervalDays(new Date(), date)) {
		case 0:
			return "今天";
		case 1:
			return "昨天";
